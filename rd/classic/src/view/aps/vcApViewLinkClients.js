Ext.define('Rd.view.aps.vcApViewLinkClients', {
    extend  : 'Ext.app.ViewController',
    alias   : 'controller.vcApViewLinkClients',
    config: {
        gridPage        : null,
        mac             : false,
    ...
    control: {
        ...
        'gridApViewLinkClients actioncolumn' : {
            itemClick  : 'onClientsActionColumnItemClick'
        }  
    },
    init: function() {
    ...
    onClickFirewall: function(record){
    	var me = this;

        if(!Ext.WindowManager.get('winApEditMacFirewallId')){
            var w = Ext.widget('winApEditMacFirewall',{id:'winApEditMacFirewallId','record' : record});
            me.getView().add(w); 
            let appBody = Ext.getBody();
            w.showBy(appBody);           
        }
    ...
    onClickLimit: function(record){
        var me = this;
        if(!Ext.WindowManager.get('winApEditMacLimitId')){
            var w = Ext.widget('winApEditMacLimit',{id:'winApEditMacLimitId','record' : record});
            me.getView().add(w); 
            let appBody = Ext.getBody();
            w.showBy(appBody);           
        } 
    ...
    onClickBlock: function(record){
    	var me = this;
       if(!Ext.WindowManager.get('winApEditMacBlockId')){
            var w = Ext.widget('winApEditMacBlock',{id:'winApEditMacBlockId','record' : record});
            me.getView().add(w); 
            let appBody = Ext.getBody();
            w.showBy(appBody);           
        }      
    ...
    onClientsActionColumnItemClick: function(view, rowIndex, colIndex, item, e, record, row, action){
        var me = this;
        var grid = view.up('grid');
        grid.setSelection(record);
        if(action == 'alias'){
            me.onClickAlias(record);
        }
        if(action == 'firewall'){
            me.onClickFirewall(record);
        }
        if(action == 'block'){
            me.onClickBlock(record);
        }
        if(action == 'limit'){
            me.onClickLimit(record);
        }         
    }
});
