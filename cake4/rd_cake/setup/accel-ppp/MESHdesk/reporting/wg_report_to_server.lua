...
require('rdWireguardJson');
require('rdLogger');

local result_file   = '/tmp/result.json'
local w_json        = rdWireguardJson();
...
local report_url    = '/cake4/rd_cake/wireguard-servers/submit-report.json';
    ...
    if(f)then
        ...
        if(r.data)then
            if(r.data.restart_service)then --Restart Service
                ...
                os.execute('/var/www/rdcore/cake4/rd_cake/setup/accel-ppp/MESHdesk/w.lua');   
            end            
        end                                    
    end
...
function lightReport()
    ...
    local stat  = w_json:showAll();
    local j_stat= cjson.encode(stat);
    --local j_stat = '{}';


    local curl_data= '{"report_type":"light","mac":"'..id..'","info":'..j_stat..'}';
    print(curl_data);
